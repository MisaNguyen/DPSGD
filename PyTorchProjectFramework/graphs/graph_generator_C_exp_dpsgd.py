...
if __name__ == "__main__":
    ...
    for idx, setting_index in enumerate(setting_indexes):
        ...
        # input()
        settings_path, Cs, sigma, s = settings_info[setting_index]["settings_path"], \
                                      settings_info[setting_index]["Cs"], \
                                      settings_info[setting_index]["sigma"], \
        ...
        # s_index_min = 1 # min = 1
        # s_index_max = 6 # max = 6
        settings = ["setting_" + str(i) for i in range(1,31)]
        ...
        if(draw_AN and idx <1):
            ...
            plt.plot(AN_BC_C_arr, AN_BC_testing_acc, "o-", label=label, color='blue',linewidth=3)
        ...
        if(draw_DPSGD_IC_case):
            ...
            # if (DGN):
            #     label = label + ", diminishing C"
            plt.plot(IC_C_arr, IC_testing_acc, "x--", label=label , color='red',linewidth=3)
            plt.subplot(1,number_of_subgraphs,number_of_subgraphs)
    ...
    plt.xlabel("C")
    plt.ylabel("accuracy")
    plt.title(" $\eta =$ %s, m = %s, $\sigma$ = %s, E =%s" % ( lr,s,sigma,epochs))
    plt.legend()
        # ic_data_path = base_path + '_IC/' + model_name + '/' + experiment + '/IC/' + setting +".json"
        #
        #
    ...
    file_name =  '/' + model_name + '_'+ mode + '_' + prefix + '_lr_' + str(lr) + '_sigma_' + str(sigma)  + '_' + str(fig_index)
