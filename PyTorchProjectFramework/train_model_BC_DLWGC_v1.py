...
def generate_private_grad(model,loss_fn,samples,targets,sigma,dict_const_Ci,device):
    ...
    # input()
    """
    ref: https://discuss.pytorch.org/t/loss-reduction-sum-vs-mean-when-to-use-each/115641/12
    if the loss function's reduction is "MEAN", then the loss will be divided by the number of elements in the batch.
    if the loss functions's reduction is "SUM", then we only divide by the number of samples in the batch *** IMPORTANT
    """
    ...
    for layer, param in model.named_parameters():
        ...
        #     print("param.grad=",param.grad.data.norm(2))
        ...
        # print("param.grad=",param.grad.data.norm(2))
        # print("noise=",torch.norm(noise, norm_type))
        # print("std=",std)
        param.grad = (grad + noise)/batch_size
        # print("param.grad=",param.grad.data.norm(2))
        # input()
    # return 0
