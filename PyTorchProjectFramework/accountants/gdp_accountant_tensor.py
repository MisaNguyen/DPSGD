...
if __name__ == '__main__':
    setting = "setting_1"
    data_path = "./graphs/data/" + setting + ".json"
    print("Running %2s with f-dp" % setting)
    with open("./dpsgd_noise.json", "r") as json_file:
        json_data = json.load(json_file)
        setting_data = json_data[setting]
        # Loading data
        epochs = setting_data["epochs"]
        noise_multiplier = setting_data["sigma"]
        num_examples = setting_data["num_examples"]
        sampling_batch = setting_data["sampling_batch"]
        # microbatches = setting_data["microbatches"]
        delta = 1/num_examples
    print("Generating data to " + data_path)

    eps_arr = []
    mu_arr = []
    for epoch in range(1,epochs+1):
        print("epoch: %3d " % (epoch))
        eps = compute_eps_poisson(epoch, noise_multiplier, num_examples,
                                  sampling_batch, delta)
        mu = compute_mu_poisson(epoch, noise_multiplier, num_examples,
                                sampling_batch)
    ...
    with open(data_path, "w") as data_file:
        out_data = {"delta": delta, "eps": eps_arr, "mu": mu_arr}
        json.dump(out_data,data_file)
        # data_file.write(str(out_data))

    print("Data is generated to " + data_path)
    print("Finished running %2s with f-dp" % setting)