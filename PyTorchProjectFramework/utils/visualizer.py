...
class Visualizer():
    ...
    def plot_current_accuracy(self, epoch, accuracy):
        """Display the current accuracy on visdom display: dictionary of error labels and values.

        Input params:
            epoch: Current epoch.
            counter_ratio: Progress (percentage) in the current epoch, between 0 to 1.
            accuracy: Training losses stored in the format of (name, float) pairs.
        """
        accuracy = {'Mnist_accuracy' : accuracy}
        if not hasattr(self, 'accuracy_plot_data'):

            self.accuracy_plot_data = {'X': [], 'Y': [], 'legend': list(accuracy.keys())}
            # self.accuracy_plot_data = {'X': [], 'Y': [], 'legend': list(losses.keys())}
        self.accuracy_plot_data['X'].append(epoch)
        self.accuracy_plot_data['Y'].append([accuracy[k] for k in self.accuracy_plot_data['legend']])
        x = np.squeeze(np.stack([np.array(self.accuracy_plot_data['X'])] * len(self.accuracy_plot_data['legend']), 1), axis=1)
        y = np.squeeze(np.array(self.accuracy_plot_data['Y']), axis=1)
        try:
            self.vis.line(
                X=x,
                Y=y,
                opts={
                    ...
                    'legend': self.accuracy_plot_data['legend'],
                    'xlabel': 'epoch',
                    'ylabel': 'accuracy'},
    ...
    def print_current_losses(self, epoch, current_sample, training_set_size, loss):
        ...
        message = 'Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
            epoch, current_sample, training_set_size,
                   100. * current_sample / training_set_size, loss.item())
